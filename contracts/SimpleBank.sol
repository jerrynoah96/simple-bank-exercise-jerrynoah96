...
contract SimpleBank {
    ...
    function deposit() public payable returns (uint) {
        /* Add the amount to the user's balance, call the event associated with a deposit,
          ...
          require(enrolled[depositor], 'this address is not enrolled');
          uint depositAmount = msg.value;
          uint currentBalance = getBalance() + depositAmount;
          currentBalance = balances[msg.sender];
          emit LogDepositMade(depositor, depositAmount);
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
        /* If the sender's balance is at least the amount they want to withdraw,
           ...
           require(balances[accountAddress] > withdrawAmount);
           msg.sender.transfer(withdrawAmount);
           uint newBalance = balances[accountAddress] - withdrawAmount;
           newBalance = balances[accountAddress];
